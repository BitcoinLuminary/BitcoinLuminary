...
def parse_config_into_btclm_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"BitcoinLuminary Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinluminary_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinluminary_config.h'), "w", encoding="utf8") as btclm_config:
        btclm_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='BitcoinLuminary-core msbuild configuration initialiser.')
    ...
    parse_config_into_btclm_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinluminary_config.h'), os.path.join(SOURCE_DIR, 'config/bitcoinluminary-config.h'))
