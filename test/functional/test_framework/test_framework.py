...
TMPDIR_PREFIX = "bitcoinluminary_func_test_"
...
class BitcoinLuminaryTestMetaClass(type):
    """Metaclass for BitcoinLuminaryTestFramework.

    Ensures that any attempt to register a subclass of `BitcoinLuminaryTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BitcoinLuminaryTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BitcoinLuminaryTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BitcoinLuminaryTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BitcoinLuminaryTestFramework(metaclass=BitcoinLuminaryTestMetaClass):
    """Base class for a bitcoinluminary test script.

    Individual bitcoinluminary test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitcoinluminaryds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bitcoinluminaryds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitcoinluminary-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_bitcoinluminaryd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinluminaryd" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinluminarycli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinluminary-cli" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinluminaryutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinluminary-util" + config["environment"]["EXEEXT"],
        )
        self.options.bitcoinluminaryd = os.getenv("BITCOINLUMINARYD", default=fname_bitcoinluminaryd)
        self.options.bitcoinluminarycli = os.getenv("BITCOINLUMINARYCLI", default=fname_bitcoinluminarycli)
        self.options.bitcoinluminaryutil = os.getenv("BITCOINLUMINARYUTIL", default=fname_bitcoinluminaryutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitcoinluminaryds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BitcoinLuminaryRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bitcoinluminaryd', self.options.bitcoinluminaryd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bitcoinluminary-cli', self.options.bitcoinluminarycli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                bitcoinluminaryd=binary[i],
                bitcoinluminary_cli=binary_cli[i],
                version=versions[i],
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple bitcoinluminaryds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitcoinluminaryd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitcoinluminaryd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitcoinluminaryd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bitcoinluminaryd=self.options.bitcoinluminaryd,
                    bitcoinluminary_cli=self.options.bitcoinluminarycli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitcoinluminary.conf
    ...
    def skip_if_no_bitcoinluminaryd_tracepoints(self):
        """Skip the running test if bitcoinluminaryd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitcoinluminaryd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoinluminaryd_zmq(self):
        """Skip the running test if bitcoinluminaryd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitcoinluminaryd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bitcoinluminary-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bitcoinluminary-wallet has not been compiled")

    def skip_if_no_bitcoinluminary_util(self):
        """Skip the running test if bitcoinluminary-util has not been compiled."""
        if not self.is_bitcoinluminary_util_compiled():
            raise SkipTest("bitcoinluminary-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitcoinluminary-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitcoinluminary-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bitcoinluminary-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bitcoinluminary-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoinluminary_util_compiled(self):
        """Checks whether bitcoinluminary-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOINLUMINARY_UTIL")
