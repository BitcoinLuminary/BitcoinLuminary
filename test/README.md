This directory contains integration tests that test bitcoinluminaryd and its
...
bitcoinluminaryd and bitcoinluminary-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bitcoinluminary-util, bitcoinluminary-tx, ...).
...
Before tests can be run locally, BitcoinLuminary Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the bitcoinluminaryd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bitcoinluminaryd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bitcoinluminaryd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bitcoinluminaryd processes are running.

On linux, the test framework will warn if there is another
bitcoinluminaryd process running when the tests are started.

If there are zombie bitcoinluminaryd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bitcoinluminaryd processes running on the system, so should not be used if any non-test
bitcoinluminaryd processes are being run.**

```bash
killall bitcoinluminaryd
```

or

```bash
pkill -9 bitcoinluminaryd
```
...
sure bitcoinluminaryd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bitcoinluminaryd `debug.log`s will all be dumped
...
`test_framework.log` and bitcoinluminaryd `debug.log`s can be combined into a single
...
call methods that interact with the bitcoinluminaryd nodes-under-test.

If further introspection of the bitcoinluminaryd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bitcoinluminaryd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bitcoinluminaryd <pid>
```
