This directory contains the source code for the BitcoinLuminary Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### bitcoinluminarygui.(h/cpp)

- Represents the main window of the BitcoinLuminary UI.
...
- ClientModel is used by the main application `bitcoinluminarygui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bitcoinluminary:... link from a browser).
...
* UI elements like BitcoinLuminaryAmountField, which inherit from QWidget.
* `bitcoinluminarystrings.cpp`: automatically generated
* `bitcoinluminaryunits.(h/cpp)`: BTCLM / mBTCLM / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bitcoinluminary_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bitcoinluminary-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bitcoinluminary-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bitcoinluminary-qt`
9. You're all set! Start developing, building, and debugging the BitcoinLuminary Core GUI
