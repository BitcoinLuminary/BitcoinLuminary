...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitcoinLuminaryUnit>("BitcoinLuminaryUnit");
#else
    qRegisterMetaType<BitcoinLuminaryUnit>("BitcoinLuminaryUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoinluminary_de.qm (shortcut "de" needs to be defined in bitcoinluminary.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoinluminary_de_DE.qm (shortcut "de_DE" needs to be defined in bitcoinluminary.qrc)
...
static const char* qt_argv = "bitcoinluminary-qt";

BitcoinLuminaryApplication::BitcoinLuminaryApplication()
...
void BitcoinLuminaryApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitcoinLuminaryApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitcoinLuminaryGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitcoinLuminaryApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitcoinLuminaryApplication::createOptionsModel(bool resetSettings)
{
...
void BitcoinLuminaryApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitcoinLuminaryGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitcoinLuminaryGUI::quitRequested, this, &BitcoinLuminaryApplication::requestShutdown);
...
void BitcoinLuminaryApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitcoinLuminaryApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitcoinLuminaryApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitcoinLuminaryApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitcoinLuminaryApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitcoinLuminaryApplication::handleRunawayException);
    connect(this, &BitcoinLuminaryApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitcoinLuminaryApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitcoinLuminaryApplication::parameterSetup()
{
...
void BitcoinLuminaryApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitcoinLuminaryApplication::requestInitialize()
{
...
void BitcoinLuminaryApplication::requestShutdown()
{
...
void BitcoinLuminaryApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // bitcoinluminary: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitcoinLuminaryGUI::handlePaymentRequest);
            connect(window, &BitcoinLuminaryGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitcoinLuminaryApplication::handleRunawayException(const QString &message)
{
...
void BitcoinLuminaryApplication::handleNonFatalException(const QString& message)
{
...
WId BitcoinLuminaryApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitcoinLuminaryApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitcoinLuminaryGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitcoinluminary);
    Q_INIT_RESOURCE(bitcoinluminary_locale);
    ...
    BitcoinLuminaryApplication app;
    ...
    /// 6-7. Parse bitcoinluminary.conf, determine network, switch to network specific
    ...
    // bitcoinluminary: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
