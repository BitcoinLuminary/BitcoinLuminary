...
#include <config/bitcoinluminary-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatBitcoinLuminaryURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(BitcoinLuminaryUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatBitcoinLuminaryURI(info));
}
