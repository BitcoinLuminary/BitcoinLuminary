...
/** Call node.updateRwSetting() with BitcoinLuminary 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BitcoinLuminary 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bitcoinluminary/bitcoinluminary/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitcoinluminary 22.x will not try to read these.
...
//! Convert enabled/size values to bitcoinluminary -prune setting.
...
//! Get pruning enabled value to show in GUI from bitcoinluminary -prune setting.
...
//! Get pruning size value to show in GUI from bitcoinluminary -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitcoinLuminaryUnit")) {
        settings.setValue("DisplayBitcoinLuminaryUnit", QVariant::fromValue(BitcoinLuminaryUnit::BTCLM));
    }
    QVariant unit = settings.value("DisplayBitcoinLuminaryUnit");
    if (unit.canConvert<BitcoinLuminaryUnit>()) {
        m_display_bitcoinluminary_unit = unit.value<BitcoinLuminaryUnit>();
    } else {
        m_display_bitcoinluminary_unit = BitcoinLuminaryUnit::BTCLM;
        settings.setValue("DisplayBitcoinLuminaryUnit", QVariant::fromValue(m_display_bitcoinluminary_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitcoinluminary.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitcoinluminary_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinLuminaryUnit>() == m_display_bitcoinluminary_unit) return;
    m_display_bitcoinluminary_unit = new_unit.value<BitcoinLuminaryUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinLuminaryUnit", QVariant::fromValue(m_display_bitcoinluminary_unit));
    Q_EMIT displayUnitChanged(m_display_bitcoinluminary_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bitcoinluminary/bitcoinluminary/pull/8273
    ...
    // (https://github.com/bitcoinluminary-core/gui/issues/567).
