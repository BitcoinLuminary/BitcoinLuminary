...
#include <qt/bitcoinluminaryaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinLuminary address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BitcoinLuminaryAddressEntryValidator(parent));
    widget->setCheckValidator(new BitcoinLuminaryAddressCheckValidator(parent));
}
...
bool parseBitcoinLuminaryURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitcoinluminary: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcoinluminary"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitcoinLuminaryUnits::parse(BitcoinLuminaryUnit::BTCLM, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinLuminaryURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitcoinLuminaryURI(uriInstance, out);
}

QString formatBitcoinLuminaryURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bitcoinluminary:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinLuminaryUnits::format(BitcoinLuminaryUnit::BTCLM, info.amount, false, BitcoinLuminaryUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitcoinLuminaryConf()
{
    ...
    /* Open bitcoinluminary.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinLuminary.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinLuminary (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BitcoinLuminary (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinLuminary*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitcoinluminary.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bitcoinluminary-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bitcoinluminary.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitcoinLuminary\n";
        else
            optionFile << strprintf("Name=BitcoinLuminary (%s)\n", chain);
