...
static void SetupBitcoinLuminaryTxArgs(ArgsManager &argsman)
{
...
{
    SetupBitcoinLuminaryTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinluminary-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  bitcoinluminary-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinluminary transaction\n"
                "or:     bitcoinluminary-tx [options] -create [commands]   Create hex-encoded bitcoinluminary transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinluminary transaction
