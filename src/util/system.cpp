...
const char * const BITCOINLUMINARY_CONF_FILENAME = "bitcoinluminary.conf";
const char * const BITCOINLUMINARY_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinluminary-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BITCOINLUMINARY_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinLuminary
    // macOS: ~/Library/Application Support/BitcoinLuminary
    // Unix-like: ~/.bitcoinluminary
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinLuminary";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinLuminary";
#else
    // Unix-like
    return pathRet / ".bitcoinluminary";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", BITCOINLUMINARY_CONF_FILENAME));
}
