# User-space, Statically Defined Tracing (USDT) for BitcoinLuminary Core

BitcoinLuminary Core includes statically defined tracepoints to allow for more
...
======================
                ...
                ┌──────────────────┐            ┌──────────────┐
                │ tracing script   │            │ bitcoinluminaryd     │
                │==================│      2.    │==============│
                │  eBPF  │ tracing │      hooks │              │
...
**Note**: BitcoinLuminary Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BitcoinLuminary Core
...
Multiple tools can list the available tracepoints in a `bitcoinluminaryd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BitcoinLuminary Core, use `info probes` in `gdb`:

```
$ gdb ./src/bitcoinluminaryd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/bitcoinluminaryd
stap net        outbound_message 0x0000000000107c05 /src/bitcoinluminaryd
stap validation block_connected  0x00000000002fb10c /src/bitcoinluminaryd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BitcoinLuminary Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitcoinluminaryd | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./src/bitcoinluminaryd -v
