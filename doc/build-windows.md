...
Below are some notes on how to build BitcoinLuminary Core for Windows.

The options known to work for building BitcoinLuminary Core on Windows are:
...
Note that for WSL the BitcoinLuminary Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoinluminary, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoinluminary`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoinluminary
