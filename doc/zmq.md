...
The BitcoinLuminary Core daemon can be configured to act as a trusted "border
router", implementing the bitcoinluminary wire protocol and relay, making
...
The ZeroMQ feature in BitcoinLuminary Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcoinluminaryd:
...
For instance:
    ...
    $ bitcoinluminaryd -zmqpubhashtx=tcp://127.0.0.1:217153 \
               -zmqpubhashtx=tcp://192.168.1.2:217153 \
               -zmqpubhashblock="tcp://[::1]:217154" \
               -zmqpubrawtx=ipc:///tmp/bitcoinluminaryd.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in bitcoinluminary.conf.
...
From the perspective of bitcoinluminaryd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcoinluminaryd directly. Furthermore, no information is
...
using. BitcoinLuminaryd appends an up-counting sequence number to each
