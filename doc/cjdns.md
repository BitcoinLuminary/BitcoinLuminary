# CJDNS support in BitcoinLuminary Core

It is possible to run BitcoinLuminary Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinLuminary Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinLuminary network and individual nodes.
...
before it will work with your BitcoinLuminary Core node. This is described in steps
...
## Run BitcoinLuminary Core with CJDNS

Once you are connected to the CJDNS network, the following BitcoinLuminary Core
...
When enabled, this option tells BitcoinLuminary Core that it is running in an
...
IPv6 local network. This helps BitcoinLuminary Core perform better address management:
...
CJDNS support was added to BitcoinLuminary Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoinluminary-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bitcoinluminary/bitcoinluminary/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BitcoinLuminary Core

There are several ways to see your CJDNS address in BitcoinLuminary Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoinluminary-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitcoinluminary-cli getpeerinfo`).
