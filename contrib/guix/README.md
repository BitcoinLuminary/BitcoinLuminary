# Bootstrappable BitcoinLuminary Core Builds

This directory contains the files necessary to perform bootstrappable BitcoinLuminary
Core builds.
...
To build BitcoinLuminary Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitcoinLuminary Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcoinluminary-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitcoinluminary-core/guix.sigs).
