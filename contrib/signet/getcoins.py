...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitcoinluminary-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitcoinluminary-cli', help='bitcoinluminary-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitcoinLuminary address to which the faucet should send')
...
parser.add_argument('bitcoinluminary_cli_args', nargs='*', help='Arguments to pass on to bitcoinluminary-cli (default: -signet)')

args = parser.parse_args()

if args.bitcoinluminary_cli_args == []:
    args.bitcoinluminary_cli_args = ['-signet']


def bitcoinluminary_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitcoinluminary_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitcoinluminary_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitcoinluminary_cli(['getnewaddress', 'faucet', 'bech32'])
